...
static void os_mem_heap_init(void)
{
    ...
    if ((heap_addr & (ALIGNMENT - 1)) !=
        0) /* If address of heap memory is not alligned	*/
    {
        heap_addr += (ALIGNMENT - 1);
        heap_addr &= ALIGNMENT;
        total_heap_size -=
            heap_addr - (uint32_t)mem_heap; /* Recalculate size of heap memory	*/
    }
        ...
        if (p_block != NULL) {
            ...
            if ((p_block->size - size) > MIN_SIZE_TO_SPLIT) {
                ...
                mem_blk_header_t *p_new_block =
                    (mem_blk_header_t *)(((uint8_t *)p_block) + SIZE_OF_BLOCK_HEADER +
                                         size);
...
void os_mem_free(void *p_addr)
{
    ...
    if ((size_t)(((uint8_t *)p_addr) - SIZE_OF_BLOCK_HEADER) <
        (size_t)mem_blk_start.next_ptr)
    {
        ...
        mem_blk_header_t *p_block =
            (mem_blk_header_t *)((uint8_t *)p_addr - SIZE_OF_BLOCK_HEADER);
        ...
        while (p_block_temp != mem_blk_end_ptr &&
               (uint8_t *)p_block_temp != (uint8_t *)p_block)
        {
        ...
        if (p_block_temp != NULL) {
            ...
            if (p_block_temp->next_ptr != NULL &&
                p_block_temp->next_ptr->state == MEM_STATE_FREE)
            {
            ...
            if (p_prev_block->state == MEM_STATE_FREE &&
                (uint8_t *)p_prev_block != (uint8_t *)&mem_blk_start)
            {
                byte_available += SIZE_OF_BLOCK_HEADER;
...
void *os_mem_malloc(size_t size)
{
    ...
    if (p_return != NULL) {
        SYS_PRINT("Addr of block is:  0x%08x\t\tSize: %d\n", (uint32_t *)p_return, size);
    }
    else {
        SYS_PRINT("Malloc failed\n");
    }
    return p_return;
}
