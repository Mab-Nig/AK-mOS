...
static list_t
    dly_task_list_2; /*< Delayed tasks (two lists are used - one for delays that have overflowed the current tick count. */
static list_t
    *volatile dly_task_list_ptr; /*< Points to the delayed task list currently being used. */
static list_t
    *volatile overflow_dly_task_list_ptr; /*< Points to the delayed task list currently being used to hold tasks that have overflowed the current tick count. */
...
static volatile uint32_t next_tick_to_unblock = (uint32_t)
    OS_CFG_DELAY_MAX; /* Initialised to portMAX_DELAY before the scheduler starts. */

static volatile uint8_t sched_is_running = (uint8_t)OS_FALSE;

uint32_t os_task_get_tick(void) { return tick_count; }
...
{
    volatile uint32_t
        *stk_ptr; /* Stack pointer, has to be the first member of TCB        */
    list_item_t
        state_list_item; /*Item in StateList include Ready, Blocked, Suspended List */
    ...
    uint32_t
        *stk_base_ptr; /* Pointer to base address of stack 					    */
    task_id_t id;
...
static void add_curr_task_to_delay_list(uint32_t tick_to_delay,
                                        uint8_t can_block_indefinitely)
{
                                  ...
                                  size_t stack_size)
{
    ...
    p_stack_ptr =
        (uint32_t *)(((uint32_t)p_stack_ptr) & (~((uint32_t)0x007))); /* Allign byte */
...
{
    tcb_high_rdy_ptr =
        list_get_owner_of_head_item(&(rdy_task_list[os_prio_get_highest()]));
...
void os_task_post_msg_dynamic(uint8_t des_task_id,
                              int32_t sig,
                              void *p_content,
                              uint8_t msg_size)
{
    ...
    case TASK_STATE_SUSPENDED_ON_MSG:
        os_msg_queue_put_dynamic(
            &(task_tcb_list[des_task_id]->msg_queue), sig, p_content, msg_size);

        /* Is the task waiting on an event ?  If so remove
         * it from the event list. */
        if (list_item_get_list_contain(&(task_tcb_list[des_task_id]->event_list_item)) !=
            NULL)
        {
    ...
    case TASK_STATE_DELAYED_ON_MSG:
        ...
        /* Is the task waiting on an event ?  If so remove
         * it from the event list. */
        if (list_item_get_list_contain(&(task_tcb_list[des_task_id]->state_list_item)) !=
            NULL)
        {
